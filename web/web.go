...
func NewServer(addr string, tsServer *ts.Server, socksAddr string, connLog *socks.ConnectionLog) *Server {
	...
	registerHandlers(mux, tsServer, socksAddr, connLog)
...
// registerHandlers sets up all HTTP routes for the Tailhopper server.
func registerHandlers(mux *http.ServeMux, tsServer *ts.Server, socksAddr string, connLog *socks.ConnectionLog) {
	// Static files
	...
	// API endpoints
	mux.Handle("/api/scan", gui.HandleScanAPI(tsServer))
